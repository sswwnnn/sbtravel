...
export default function DetailTripOfDriver() {
  ...
  return <div>
    ...
    <div style={{ margin: 20 }} >
      ...
      <Modal
        title={"Driver Information:  "+driverDetail?.fullName}
        centered
        open={open}
        onOk={() => setOpen(false)}
    ...
    <Table dataSource={tripNow} columns={columns} sort={false}  pagination={false}/>
      <div className='text-center text-red-500 font-sans mt-5'><Tag color="" style={{ fontSize:"20px"}}> Next departure time: {s!=0?<>{d}(d)--{h}(h)--{m}(m)--{s}(s)</>:"There is no upcoming schedule."} </Tag></div>
